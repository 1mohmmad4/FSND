...
def joinedPeginate(questions, page):
    start = QUESTIONS_PER_PAGE * (page - 1)
    end = start + QUESTIONS_PER_PAGE
...
def create_app(test_config=None):
  ...
  '''
    ...
    def getQuestions():
        ...
        questionsJson = peginate(questions, page)
  ...
  '''
    ...
    def getQuestionsByCategory(categoryId):
        questions = db.session.query(Question, Category).join(
            Category, Question.category == Category.type).filter(Category.id == categoryId).all()

        page = request.args.get('page', 1, type=int)

        questionsJson = joinedPeginate(questions, page)
  ...
  '''
    ...
    def getQuiz():
        ...
        if category is None:
            ...
            question = query.filter(
                Question.category == category, Question.id.notin_(previousQuestionIds)).first()
